...
# Parse XML
f = open("data/drugbank/drugbank_xml2csv_python/drugbank_2019_11.xml", "r")
data = f.read()
f.close()
...
for child in root:
    ...
    groups = [
        s.text for s in child.find(tag_prefix + "groups").findall(tag_prefix + "group")
    ]
    ...
    for target in child.find(tag_prefix + "targets").findall(tag_prefix + "target"):
        ...
        target_gene = (
            target.find(tag_prefix + "polypeptide").find(tag_prefix + "gene-name").text
        )
        target_name = target.find(tag_prefix + "name").text
        target_organism = target.find(tag_prefix + "organism").text
        target_taxonomy_id = (
            target.find(tag_prefix + "polypeptide")
            .find(tag_prefix + "organism")
            .attrib["ncbi-taxonomy-id"]
        )
        ...
        target_external_ids = (
            target.find(tag_prefix + "polypeptide")
            .find(tag_prefix + "external-identifiers")
            .findall(tag_prefix + "external-identifier")
        )
        target_uniprot_id = ""
        target_genbank_gene = ""
        ...
        for external_id in target_external_ids:
            if external_id.find(tag_prefix + "resource").text == "UniProtKB":
                ...
                external_id.find(tag_prefix + "resource").text
                ...
                target_genbank_protein = external_id.find(
        ...
        target_actions = [
            s.text.lower()
            for s in target.find(tag_prefix + "actions").findall(tag_prefix + "action")
        ]

        drug2attrib[drugbank_id]["targets"].append((target_id, target_actions))

        if (
            target_id not in target2attrib
        ):  # {'gene', 'name', 'organism', 'taxonomy_id', 'uniprot_id', 'genbank_gene_id', 'genbank_protein_id', 'hgnc_id'}
    ...
    for enzyme in child.find(tag_prefix + "enzymes").findall(tag_prefix + "enzyme"):
        ...
        enzyme_gene = (
            enzyme.find(tag_prefix + "polypeptide").find(tag_prefix + "gene-name").text
        )
        enzyme_name = enzyme.find(tag_prefix + "name").text
        enzyme_organism = enzyme.find(tag_prefix + "organism").text
        enzyme_taxonomy_id = (
            enzyme.find(tag_prefix + "polypeptide")
            .find(tag_prefix + "organism")
            .attrib["ncbi-taxonomy-id"]
        )
        ...
        for external_id in enzyme_external_ids:
            if external_id.find(tag_prefix + "resource").text == "UniProtKB":
                ...
                enzyme_genbank_protein = external_id.find(
                    tag_prefix + "identifier"
                ).text
            elif (
                external_id.find(tag_prefix + "resource").text
        ...
        enzyme_actions = [
            s.text.lower()
            for s in enzyme.find(tag_prefix + "actions").findall(tag_prefix + "action")
        ]

        drug2attrib[drugbank_id]["enzymes"].append((enzyme_id, enzyme_actions))

        if (
            enzyme_id not in enzyme2attrib
        ):  # {'gene', 'name', 'organism', 'taxonomy_id', 'uniprot_id', 'genbank_gene_id', 'genbank_protein_id', 'hgnc_id'}
    ...
    for transporter in child.find(tag_prefix + "transporters").findall(
        tag_prefix + "transporter"
    ):
        ...
        if transporter_organism == "Homo sapiens":
            transporter_organism = "Human"
        if (
            transporter_gene is None
            or transporter_organism is None
            or transporter_taxonomy_id is None
        ):
            continue

        transporter_external_ids = (
            transporter.find(tag_prefix + "polypeptide")
            .find(tag_prefix + "external-identifiers")
            .findall(tag_prefix + "external-identifier")
        )
        ...
        for external_id in transporter_external_ids:
            if external_id.find(tag_prefix + "resource").text == "UniProtKB":
                transporter_uniprot_id = external_id.find(
                    tag_prefix + "identifier"
                ).text
            elif (
                external_id.find(tag_prefix + "resource").text
                ...
                transporter_genbank_gene = external_id.find(
                    tag_prefix + "identifier"
                ).text
            elif (
                external_id.find(tag_prefix + "resource").text
                ...
                transporter_genbank_protein = external_id.find(
                    tag_prefix + "identifier"
                ).text
            elif (
                external_id.find(tag_prefix + "resource").text
        ...
        transporter_actions = [
            s.text.lower()
            for s in transporter.find(tag_prefix + "actions").findall(
                tag_prefix + "action"
            )
        ]

        drug2attrib[drugbank_id]["transporters"].append(
            (transporter_id, transporter_actions)
        )

        if (
            transporter_id not in transporter2attrib
        ):  # {'gene', 'name', 'organism', 'taxonomy_id', 'uniprot_id', 'genbank_gene_id', 'genbank_protein_id', 'hgnc_id'}
    ...
    print(
        "targets:", len(drug2attrib[drugbank_id]["targets"]),
    )
    print(
        "enzymes:", len(drug2attrib[drugbank_id]["enzymes"]),
    )
...
for drugbank_id in sorted(drug2attrib.keys()):
    if (
        len(drug2attrib[drugbank_id]["targets"]) == 0
        and len(drug2attrib[drugbank_id]["enzymes"]) == 0
        and len(drug2attrib[drugbank_id]["transporters"]) == 0
    ):
        continue
    else:
...
for partner_id in sorted(transporter2attrib.keys()):
    ...
    if taxonomy_id == "9606" and organism.lower() != "human":
        print(
            partner_id,
            transporter2attrib[partner_id]["gene"],
            organism,
            taxonomy_id,
            "organism mismatch",
        )
...
for drugbank_id in drugs:
    for (target_id, target_actions) in drug2attrib[drugbank_id]["targets"]:
        ...
        if (
            target2attrib[target_id]["organism"] == "Human"
            and target2attrib[target_id]["taxonomy_id"] == "9606"
        ):
...
for drugbank_id in drugs:
    for (enzyme_id, enzyme_actions) in drug2attrib[drugbank_id]["enzymes"]:
        ...
        if (
            enzyme2attrib[enzyme_id]["organism"] == "Human"
            and enzyme2attrib[enzyme_id]["taxonomy_id"] == "9606"
        ):
...
for drugbank_id in drugs:
    for (transporter_id, transporter_actions) in drug2attrib[drugbank_id][
        "transporters"
    ]:
        actions = [
            1 if action in transporter_actions else 0
            for action in transporter_actions_to_write
        ]

        writer.writerow([drugbank_id, transporter_id] + actions)

        if (
            transporter2attrib[transporter_id]["organism"] == "Human"
            and transporter2attrib[transporter_id]["taxonomy_id"] == "9606"
        ):
